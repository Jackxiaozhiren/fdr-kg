...
Specifically, we:
\begin{enumerate}
    \item Sample $K$ negative tail entities uniformly from $\mathcal{E}$, excluding the ground-truth tail $t^*_i$ and all tails known to form valid triples with $(h_i, r_i)$ in the training set (filtered setting).
...
For comparison, model training requires 15--25 minutes (20 epochs), so the FDR pipeline adds roughly 15--33\% overhead relative to training---a modest cost given that it requires no model modification or retraining.
