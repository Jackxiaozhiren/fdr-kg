...
\midrule
TransE & 0.511 & 0.283 & 1{,}447 (49.5\%) & 1{,}884 (64.4\%) & 1{,}447 (49.5\%) \\
...
\begin{enumerate}
    ...
    \item Under the BH procedure, rejection rates at $\alpha = 0.05$ range from $0.0\%$ (ComplEx) to $49.5\%$ (TransE)---a near-complete span of the possible discovery spectrum---showing that model reliability varies dramatically in ways MRR alone does not capture.
    \item ComplEx registers the highest $\hat{\pi}_0$ (0.754) and zero BH rejections at $\alpha = 0.05$ on WN18RR, indicating its embedding scores carry essentially no signal for distinguishing true triples from random negatives at $K = 100$.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig3_mrr_vs_fdr.pdf}
\caption{MRR vs.\ BH rejection rate at $\alpha = 0.05$ for all four models on WN18RR (circles) and FB15k-237 (squares). The two metrics exhibit substantial scatter: models with similar MRR can differ markedly in statistical reliability. TransE and RotatE illustrate the complementarity on WN18RR---RotatE leads on MRR (0.589 vs.\ 0.511) while TransE yields 49.5\% BH discoveries vs.\ 37.6\%. On FB15k-237, all models cluster at low BH rejection rates (1.8\%--10.9\%), reflecting the benchmark's greater difficulty.}
\label{fig:model-comp}
\end{figure}

\textbf{RQ3 summary.}
\begin{enumerate}
    \item MRR ranking: RotatE ($0.589$) $>$ TransE ($0.511$) $>$ ConvE ($0.469$) $>$ ComplEx ($0.074$).
    \item FDR-based ranking (BH rejection at $\alpha = 0.05$): TransE ($49.5\%$) $>$ RotatE ($37.6\%$) $>$ ConvE ($25.4\%$) $>$ ComplEx ($0.0\%$).
    \item MRR and FDR provide complementary but distinct model rankings: RotatE leads on MRR yet TransE leads on FDR power---a difference of 11.9 percentage points in BH rejection rate despite a 0.078 gap in MRR. ConvE and ComplEx rank consistently (third and fourth, respectively) on both metrics, indicating that the two evaluation dimensions highlight different aspects of model quality rather than always diverging.
    \item The TransE--RotatE comparison illustrates the value of multi-dimensional evaluation. RotatE achieves higher MRR (better aggregate ranking) yet TransE yields $1.3\times$ more statistically significant discoveries at the same FDR threshold. A practitioner selecting a model solely by MRR would miss this dimension of reliability.
\end{enumerate}

The FDR framework reveals that model ranking depends on the evaluation objective: ranking quality (MRR) and statistical reliability (FDR) capture distinct dimensions of model behavior, and neither metric alone provides a complete characterization of model quality for knowledge-based system deployment.
...
\begin{enumerate}
    ...
    \item \textbf{FDR rankings partially diverge from MRR.} On FB15k-237, TransE and ConvE swap positions between the MRR-based ordering (RotatE $>$ ConvE $>$ TransE $>$ ComplEx) and the FDR-based ordering (RotatE $>$ TransE $>$ ConvE $>$ ComplEx). This confirms that MRR and FDR capture partially distinct dimensions of model quality, though the ranking divergence pattern differs from WN18RR---TransE's FDR advantage relative to ConvE is consistent across both datasets, reflecting stable architectural differences in score calibration.
...
The experiments indicate that the proposed FDR framework functions as a diagnostic instrument that characterizes the reliability profile of any KG embedding model, rather than merely providing an alternative evaluation metric.
It identifies three properties directly relevant to knowledge-based system deployment:
\begin{itemize}
    \item \textbf{Noise level:} $\hat{\pi}_0$ estimates the fraction of test triples for which the model carries no predictive signal above background noise, quantifying the model's fundamental reliability ceiling.
    \item \textbf{Relation-level reliability:} Per-relation FDR analysis reveals which relations the model handles with statistical confidence and which remain at chance level, enabling targeted quality assurance for high-stakes relation types.
    \item \textbf{Model selection guidance:} Cross-model FDR comparison exposes cases where higher MRR does not translate to higher statistical reliability, directly informing model choice for deployment in knowledge-based systems.
\end{itemize}

\textbf{Implications for knowledge-based system deployment.}
For a practitioner deploying KG completion in a safety-critical setting, the FDR framework provides actionable information that MRR cannot.
$\hat{\pi}_0$ quantifies the proportion of the model's output that should be treated as noise rather than knowledge---a direct input to system risk assessment.
Per-relation diagnostics enable selective deployment: a model may be judged reliable for certain relation types while its predictions for other relations are routed for human review.
The cross-model comparison reveals that selecting a model by MRR alone may yield suboptimal reliability: on WN18RR, a practitioner choosing RotatE for its higher MRR ($0.589$ vs.\ TransE's $0.511$) would deploy a model that produces $31\%$ fewer statistically significant discoveries at FDR $\leq 0.05$ ($37.6\%$ vs.\ $49.5\%$).
These are the kinds of trade-offs that knowledge-based system designers must reason about, and they are invisible to traditional evaluation.

\textbf{Ablation of FDR components.}
To assess the incremental contribution of each FDR component, we compare three diagnostic layers independently. Table~\ref{tab:fdr-overall} shows that these components yield complementary information: BH provides conservative but guaranteed error control under the PRDS condition (see \S\ref{subsec:bh-procedure}), the $q$-value framework leverages $\hat{\pi}_0$ estimation to increase discovery power (e.g., TransE: $49.5\%$ BH vs.\ $64.4\%$ $q$-value, RotatE: $37.6\%$ BH vs.\ $70.1\%$ $q$-value), and the local FDR adds per-triple calibration. For ComplEx on WN18RR, all three methods agree: BH and $q$-value both report zero discoveries, and only 20 triples ($0.7\%$) pass the local FDR threshold---consistent with the model's $\hat{\pi}_0 = 0.754$ and near-random MRR of $0.074$.
...
Fourth, the current evaluation reports results from single training runs at 20 epochs per model. A supplementary multi-seed analysis across TransE, RotatE, and ConvE under two additional random seeds ($\{123, 456\}$, matching the 20-epoch primary protocol) confirms that $\hat{\pi}_0$ and BH rejection rates are stable across initializations (coefficient of variation below $6\%$ for $\hat{\pi}_0$ and below $2\%$ for BH rejection rate across all three models), indicating that single-seed FDR comparisons are reliable and the observed cross-model differences are driven by model architecture rather than random initialization. Extending multi-seed evaluation to ComplEx and to FB15k-237 would further strengthen these conclusions.
